...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        img = cv2.resize(img, self.image_size,  cv2.INTER_LINEAR).astype(np.float32)
        mask = cv2.resize(mask, self.image_size, cv2.INTER_LINEAR).astype(np.float32)
...
class Normalize_test(object):
    def __init__(self, image_size=(224, 320)):
        self.image_size = image_size
