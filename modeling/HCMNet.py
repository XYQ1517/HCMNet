...
class DWT_Function(Function):
    ...
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):
        ...
        x_H = torch.cat([x_ll, x_lh, x_hl, x_hh], dim=1)
        return x_H, x_ll

    @staticmethod
    def backward(ctx, dx, _):
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y1_1, y2_1, y3_1, y4_1 = self.forward_core(x2)
        assert y1.dtype == torch.float32
        y_1 = y1_1 + y2_1 + y3_1 + y4_1
        y_1 = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)

        y = self.out_norm(y+y_1)
        y = y * F.silu(z)
        out = self.out_proj(y)
...
class IMamba(nn.Module):
    def __init__(self, in_c, k_size = 3, mlp_ratio=4., drop=0., norm_layer=nn.LayerNorm):
        ...
        self.FFN = FFN(in_features=in_c, hidden_features=mlp_hidden_dim, drop=drop)

    def forward(self, x):
        input = x
        x1 = self.act(self.ins_norm(x))
        x1 = self.conv(x1)
        x1 = self.block(x1.permute(0, 2, 3, 1))
        x1 = x1 + input 

        x2_n = self.norm(x1.permute(0, 2, 3, 1))
        x2 = self.FFN(x2_n)
        x2 = x2 + x1 
        return x2
...
class HCMNet(nn.Module):
    def __init__(self, num_classes=1, head_in_channel=256):
        ...
        self.filters = [3, 64, 128, 256, 512]
        self.make_wavelet = make_wavelet(self.filters)
